...
--#1 NEED TO FINISH - testing w Newark to get write answer
...
FROM Customers
WHERE cid IN (SELECT cid, Coalesce ((SUM(totalUSD)), 'NULL')
		FROM Orders
		);
...
--6 need to check if any columns
SELECT Orders.totalUSD
FROM Orders, Customers, Products
WHERE Orders.cid = Customers.cid
AND Orders.pid = Products.pid
AND totalUSD in (SELECT totalUSD
			FROM Orders
			GROUP BY totalUSD
			HAVING totalUSD = SUM((Orders.qty*Products.priceUSD) - ((Orders.totalUSD*Customers.discount)/100))
			);
