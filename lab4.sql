...
--#4 did 2 statements b/c looking at separate customers
SELECT DISTINCT cid
FROM Orders
WHERE cid in (SELECT cid
		FROM orders
...
--#5 no a07 seller so it should be all
SELECT DISTINCT pid
FROM Orders
WHERE cid in (SELECT cid
		FROM Orders
...
--#6 - no london b/c a08 instead of a07 in cap3
...
--#7 no customers have the same discount as Dallas/London
SELECT *
FROM Customers AS c
WHERE discount in (SELECT discount
			FROM Customers AS cc
			WHERE c.cid != cc.cid
			AND c.discount = cc.discount
			AND c.city in ('Dallas', 'London')
			);
